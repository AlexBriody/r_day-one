...
```{r}
# "nycflights13," "gapminder," and "Lahman," are R packages that provide datasets for use in learning and practicing data analysis, visualization, and statistical modeling.
# nycflights13: contains data about flights departing from New York City airports in 2013. 
# The Gapminder project, which tracks global socioeconomic indicators over time.
# The Lahman package cotains historical baseball data. 
install.packages(c('nycflights13', 'gapminder', 'Lahman'))

# Command is used in R to load the "nycflights13" package into your R session
library(nycflights13)

# Notation accesses the flights dataset in the "nycflights13" package, without having to load the entire package.
...
mutated_data <- mutate(mtcars,disp/cyl)
summarize(mtcars, mean_mpg = mean(mpg, na.rm = TRUE))
...
f <- flights %>%
  ...
  summarize(mean = mean(arr_delay, na.rm = TRUE), count = n())
f
...
```{r}
ggplot(data=myplot, mapping = aes(displ, hwy, colour = class)) +
  geom_point()
```
...
```{r}
ggplot(myplot, mapping = aes(displ, hwy, colour = drv)) +
...
```{r}
read_csv("/Users/alexanderbriody/Desktop/R-8-22-23/femaleMiceWeights.csv")
```
...
my_data <- read.csv("/Users/alexanderbriody/Desktop/R-8-22-23/femaleMiceWeights.csv") %>% as_tibble()
my_data
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    duty_free <- function(price, discount, holiday_cost) {
      ...
      bottles_needed <- floor(holiday_cost/ savings_per_bottle)
      return(bottles_needed)
    }

    # Problem 3
    twice_as_old <- function(dad_years_old, son_years_old) {
      age_difference <- abs(dad_years_old - 2 * son_years_old)
...
deaths <- seatbelts_df  %>%
  ...
  summarize(
    count = n(),
    mean_distance = mean(kms, na.rm = TRUE),
    min_distance = min(kms, na.rm = TRUE),
    max_distance = max(kms, na.rm = TRUE),
  ) %>%
  filter(mean_distance > 10000)
...
ggplot(data = deaths, aes(x = mean_distance, y = DriversKilled)) +
  geom_point() +
  labs(title = "Scatter Plot: Mean Distance Driven vs. Drivers Killed",
       x = "Mean Distance Driven (kms)",
       y = "Number of Drivers Killed")
```

```{r}
# The scatterplot shows a negative correlation between the mean distance driven and the number of drivers killed. As the mean distance driven increases, the number of drivers killed decrease. This may due to the possibility that more experienced drivers tend to drive longer distances and have learned to be cautious.
```
